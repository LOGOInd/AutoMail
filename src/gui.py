...
class Frame2(object):
    ...
    def load(self):
        self.frame = tk.Frame(self.root, width=self.root.winfo_screenwidth(
        )-400, height=self.root.winfo_screenheight(), bg="#fff")
        self.frame.pack(anchor=tk.N, fill=tk.X, expand=True, side=tk.TOP)
        # self.frame.grid(row=2, column=0)

        self.stepname = tk.Label(self.frame, text="2.Upload the Certificate.", font=(
            "Helvetica", 36), fg="#4f4f4f")
        ...
        self.desc_1_1 = tk.Label(self.frame, text="Attach an image file", font=(
            "Helvetica", 18), fg="#4f4f4f", height=2, bg=None)
        self.desc_1_1.pack(side=tk.TOP)

        # img=ImageTk.PhotoImage(Image.open(dir_path+"/src/assets/button_1_1.png"))
        self.button1_1 = tk.Button(self.frame, text="Upload Certificate",
                                   image=None, command=self.fetchfile, height=2, width=14)
        self.button1_1.pack()

        self.desc_1_2 = tk.Label(self.frame, text='No File Choosen', font=(
            "Helvetica", 12), fg="#4f4f4f", height=2, bg=None)
        self.desc_1_2.pack(side=tk.TOP)

        self.imgcanvas = tk.Canvas(
            self.frame, width=200, height=200, bg='#000')
        self.imgcanvas.pack()
        ...
        self.button1_2 = tk.Button(
            self.frame, text="Next", image=None, command=self.nextpage, height=2, width=12)
    ...
    def fetchfile(self):
        self.filepath = tk.filedialog.askopenfilename(parent=self.root, filetypes=[(
            "Jpeg", "*.jpg"), ("Jpeg", "*.jpeg"), ("Png", "*.png"), ("Png", "*.bmp")])
...
class Frame1(object):
    ...
    def load(self):
        self.filepath = None
        self.frame = tk.Frame(self.root, width=self.root.winfo_screenwidth(
        )-400, height=self.root.winfo_screenheight()-350, bg="#fff")
        self.frame.pack(anchor=tk.N, fill=tk.X, expand=True, side=tk.TOP)
        # self.frame.grid(row=2, column=0)

        self.stepname = tk.Label(self.frame, text="1. Get the mailing list.", font=(
            "Helvetica", 36), fg="#4f4f4f")
        self.stepname['bg'] = self.stepname.master['bg']
        self.stepname.pack(side=tk.TOP)
        ...
        self.desc_1_1 = tk.Label(self.frame, text="Attach a .csv or a .xlsx file", font=(
            "Helvetica", 18), fg="#4f4f4f", height=2, bg=None)
        self.desc_1_1.pack(side=tk.TOP)

        # img=ImageTk.PhotoImage(Image.open(dir_path+"/src/assets/button_1_1.png"))
        self.button1_1 = tk.Button(
            self.frame, text="Upload a file", image=None, command=self.fetchfile, height=2, width=12)
        self.button1_1.pack()
        ...
        self.button1_2 = tk.Button(
            self.frame, text="Next", image=None, command=self.validate_csv, height=2, width=12)
        self.button1_2["state"] = "disabled"
        self.button1_2.pack()
    ...
    def fetchfile(self):
        self.filepath = tk.filedialog.askopenfilename(parent=self.root, filetypes=[(
            "Comma Seperated Values", "*.csv"), ("Excel File", "*.xls"), ("Excel File", "*.xlsx")])
        self.filename = self.filepath.split("/")[-1]
        self.desc_1_2.configure(text=str("File Choosen: "+self.filename))
        if len(self.filename) != 0:
            self.button1_2["state"] = "normal"
    ...
    def raise_validate_error(self, error):
        ...
        self.checkerLabel = tk.Label(
            self.checker, text="Checking File", fg="#4f4f4f")
        ...
        self.checkerButton = tk.Button(
            self.checker, text="Retry", image=None, command=self.checker.destroy, height=2, width=12)
