...
rect_id = None
path = os.path.join(os.getcwd(), "src", "Certificate.jpg")
# path = ''
...
label = tk.Label(
    window, text="Select the portion where name has to be written.", font=("Helvetica", 18))
label.pack()

img = ImageTk.PhotoImage(Image.open(rsImgLoc))

# img = ImageTk.PhotoImage(image=Image.fromarray(rsImgLoc))
...
rect_id = canvas.create_rectangle(
    topx, topy, topx, topy, dash=(2, 2), fill='', outline='#000')

canvas.bind('<Button-1>', get_mouse_posn)
canvas.bind('<B1-Motion>', update_sel_rect)

button = tk.Button(window, text="OK", image=None,
                   command=lambda: save_location(window), height=2, width=14)
button.pack()
